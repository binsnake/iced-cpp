...
std::vector<HandlerEntry> read_legacy_tables() {
  ...
  static std::vector<HandlerEntry> cached_table = []() {
    TableDeserializer deserializer(
      std::span<const uint8_t>( g_legacy_tbl_data, sizeof(g_legacy_tbl_data)),
      LEGACY_MAX_ID_NAMES,
...
std::vector<std::vector<HandlerEntry>> read_vex_tables() {
  ...
  static std::vector<std::vector<HandlerEntry>> cached_tables = []() {
    TableDeserializer deserializer(
      std::span<const uint8_t>( g_vex_tbl_data, sizeof(g_vex_tbl_data)),
      VEX_MAX_ID_NAMES,
      read_vex_handlers
    );
    deserializer.deserialize();
...
std::vector<std::vector<HandlerEntry>> read_evex_tables() {
  ...
  static std::vector<std::vector<HandlerEntry>> cached_tables = []() {
    TableDeserializer deserializer(
      std::span<const uint8_t>( g_evex_tbl_data, sizeof(g_evex_tbl_data)),
      EVEX_MAX_ID_NAMES,
      read_evex_handlers
    );
    deserializer.deserialize();
